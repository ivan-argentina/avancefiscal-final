...
export default function AbmArticulos() {
  ...
  const calcularCostoDesdePrecio = (precioVenta, porcentaje) => {
    const precioNum = Number(precioVenta) || 0;
    const margenNum = Number(porcentaje) || 0;

    if (!precioNum) return "";

    return (precioNum / (1 + margenNum / 100)).toFixed(2);
  };
  ...
  const manejarCambioPrecio = (valor) => {
    ...
    if (margen !== "") {
      const nuevoCosto = calcularCostoDesdePrecio(valor, margen);
      setPrecioCosto(nuevoCosto);
    }
  };
          ...
          <Grid container spacing={2}>
            ...
            <Grid size={{ xs: 12, md: 2 }}>
              <TextField
                ...
                onBlur={() => buscarArticuloPorCodigo(codigo)}
                inputRef={codigoRef}
                onKeyDown={(e) => {
                  if (e.key === "Enter") {
